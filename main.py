...
def chat(**kwargs):
    ...
    searcher, sos, eos, unknown, word2ix, ix2word = train_val.test(opt)
    ...
    while(1):
        ...
        if opt.use_QA_first:
            ...
            if(query_res == tuple()):
                output_words = train_val.output_answer(input_sentence, searcher, sos, eos, unknown, opt, word2ix, ix2word)
            else:
